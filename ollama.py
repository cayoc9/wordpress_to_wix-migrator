...
REGRAS OBRIGATÓRIAS:
1. Retorne SOMENTE um objeto JSON válido com as chaves "excerpt", "metaDescription" e "seoTitle"
...
5. O seoTitle deve ter no máximo 60 caracteres, ser otimizado para SEO e mais impactante que o título original
...
CONTEÚDO: {text}"""
    try:
        ...
        if data is None:
            ...
            return {"excerpt": "", "metaDescription": "", "seoTitle": ""}
...
# Carregar o CSV
df = pd.read_csv("Posts-Export-2025-July-25-1838.csv")
...
for index, row in df.iterrows():
    ...
    print(f"\n=== POST {index + 1} ===")
    print(f"Title: {title}")
    print(f"ImageURL raw: {repr(row['Image URL'])}")
    print(f"ImageURL type: {type(row['Image URL'])}")
    
    # Tratamento seguro para ImageURL
    if pd.notna(row['Image URL']) and str(row['Image URL']).strip():
        image_url = str(row['Image URL']).split("|")[0]
    ...
    print(f"SEO Title: {result['seoTitle']}")
    ...
    df.at[index, 'SeoTitle'] = result['seoTitle']
