...
# --- Testes para o script_handler ---

def test_handle_script_with_rdstation_src():
    """Testa o handler para script com src contendo 'rdstation'."""
    html = '''<div id="ajuda-com-codigo-de-barras-b6e0d0f0c73e83ced796" role="main"></div>
<script type="text/javascript" src="https://d335luupugsy2.cloudfront.net/js/rdstation-forms/stable/rdstation-forms.min.js"></script>
<script type="text/javascript">
  new RDStationForms('ajuda-com-codigo-de-barras-b6e0d0f0c73e83ced796-html', 'UA-115444208-1').createForm();
</script>'''
    
    ricos = convert_html_to_ricos(html)
    
    # Deve ter 2 nós: div (como HTML fallback), e um nó de parágrafo contendo os scripts
    assert len(ricos["nodes"]) == 2
    
    # Primeiro nó é o div convertido para HTML
    assert ricos["nodes"][0]["type"] == "HTML"
    
    # Segundo nó é um parágrafo contendo os scripts como texto
    assert ricos["nodes"][1]["type"] == "PARAGRAPH"
    # Os scripts estão sendo tratados como texto dentro do parágrafo

def test_handle_script_without_rdstation():
    """Testa o handler para script sem 'rdstation' no src."""
    html = '''<script type="text/javascript" src="https://example.com/script.js"></script>
<script type="text/javascript">
  console.log("Hello World");
    ...
    # Deve ter 1 nó: parágrafo contendo os scripts como texto
    assert len(ricos["nodes"]) == 1
    
    # O nó é um parágrafo contendo os scripts como texto
    assert ricos["nodes"][0]["type"] == "PARAGRAPH"

def test_handle_inline_script():
    """Testa o handler para script inline (sem src)."""
    html = '''<script type="text/javascript">
  new RDStationForms('form-id', 'UA-115444208-1').createForm();
</script>'''
    
    ricos = convert_html_to_ricos(html)
    
    # Deve ter 1 nó: parágrafo contendo o script como texto
    assert len(ricos["nodes"]) == 1
    
    # O nó é um parágrafo contendo o script como texto
    assert ricos["nodes"][0]["type"] == "PARAGRAPH"