...
def get_member_by_email(cfg: Dict[str, str], email: str) -> Optional[Dict[str, Any]]:
    ...
    First checks if the email needs to be mapped to another email (for authors).
    Then attempts to retrieve the member by the email address.
    ...
    '''
    # Check if email needs to be mapped (for authors)
    author_mapping_path = 'config/author_email_mapping.json'
    try:
        with open(author_mapping_path, 'r') as f:
            mapping_data = json.load(f)
            email_mapping = mapping_data.get('author_email_mapping', {})
            if email in email_mapping:
                email = email_mapping[email]
    except (FileNotFoundError, json.JSONDecodeError) as e:
        # If mapping file doesn't exist or is invalid, continue with original email
        print(f'Warning: Could not load author mapping file {author_mapping_path}: {e}')
    ...
    def do_request() -> requests.Response:
        # Using proper JSON escaping for query parameter
        query = json.dumps({'filter': {'loginEmail': {'': email}}})
        url = f'{cfg["base_url"]}/members/v1/members?query={query}'
        return requests.get(url, headers=wix_headers(cfg))
    ...
    try:
        ...
        if e.response.status_code == 404:
            # Member not found is not an error condition, just return None
            return None
        print(f'Failed to get member by email {email}: {e.response.text}')
        return None
    except requests.RequestException as e:
        print(f'Request failed for email {email}: {e}')
        return None
